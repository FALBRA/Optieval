...
var CdkTable = class _CdkTable {
  ...
  _onDestroy = new import_rxjs4.Subject();
   ...
   * @docs-private
   */
  viewChange = new import_rxjs4.BehaviorSubject({
    start: 0,
  ...
  _observeRenderChanges() {
    ...
    } else if ((0, import_rxjs4.isObservable)(this.dataSource)) {
      dataStream = this.dataSource;
    } else if (Array.isArray(this.dataSource)) {
      dataStream = (0, import_rxjs4.of)(this.dataSource);
    }
  ...
  _setupStickyStyler() {
    ...
    (this._dir ? this._dir.change : (0, import_rxjs4.of)()).pipe((0, import_operators2.takeUntil)(this._onDestroy)).subscribe((value) => {
...
var InputModalityDetector = class _InputModalityDetector {
  ...
  _modality = new import_rxjs7.BehaviorSubject(null);
...
var FocusMonitor = class _FocusMonitor {
  ...
  _stopInputModalityDetector = new import_rxjs7.Subject();
  constructor() {
...
var BreakpointObserver = class _BreakpointObserver {
  ...
  _destroySubject = new import_rxjs8.Subject();
  constructor() {
  }
  ...
  observe(value) {
    ...
    let stateObservable = (0, import_rxjs8.combineLatest)(observables);
    stateObservable = (0, import_rxjs8.concat)(stateObservable.pipe((0, import_operators4.take)(1)), stateObservable.pipe((0, import_operators4.skip)(1), (0, import_operators4.debounceTime)(0)));
  ...
  _registerQuery(query) {
    ...
    const queryObservable = new import_rxjs8.Observable((observer) => {
...
var ContentObserver = class _ContentObserver {
  ...
  observe(elementOrRef) {
    ...
    return new import_rxjs9.Observable((observer) => {
      ...
      const subscription = stream.pipe((0, import_operators5.map)((records) => records.filter((record) => !shouldIgnoreRecord(record))), (0, import_operators5.filter)((records) => !!records.length)).subscribe((records) => {
        this._ngZone.run(() => {
  ...
  _observeElement(element) {
    return this._ngZone.runOutsideAngular(() => {
      if (!this._observedElements.has(element)) {
        const stream = new import_rxjs9.Subject();
...
var CdkObserveContent = class _CdkObserveContent {
  ...
  _subscribe() {
    ...
    this._currentSubscription = (this.debounce ? stream.pipe((0, import_operators5.debounceTime)(this.debounce)) : stream).subscribe(this.event);
  }
  _unsubscribe() {
...
var Typeahead = class {
  _letterKeyStream = new import_rxjs10.Subject();
  _items = [];
  _selectedItemIndex = -1;
  /** Buffer for the letters that the user has pressed */
  _pressedLetters = [];
  _skipPredicateFn;
  _selectedItem = new import_rxjs10.Subject();
  ...
  _setupKeyHandler(typeAheadInterval) {
    this._letterKeyStream.pipe((0, import_operators6.tap)((letter) => this._pressedLetters.push(letter)), (0, import_operators6.debounceTime)(typeAheadInterval), (0, import_operators6.filter)(() => this._pressedLetters.length > 0), (0, import_operators6.map)(() => this._pressedLetters.join("").toLocaleUpperCase())).subscribe((inputString) => {
...
function coerceObservable(data) {
  if (!(0, import_rxjs12.isObservable)(data)) {
    return (0, import_rxjs12.of)(data);
  }
  return data;
}
...
var TreeKeyManager = class {
  ...
  _items = [];
  _typeahead;
  _typeaheadSubscription = import_rxjs13.Subscription.EMPTY;
  ...
  constructor(items, config) {
    ...
    } else if ((0, import_rxjs13.isObservable)(items)) {
  ...
  change = new import_rxjs13.Subject();
  ...
  _expandCurrentItem() {
    ...
    if (!this._isCurrentItemExpanded()) {
      ...
      coerceObservable(this._activeItem.getChildren()).pipe((0, import_operators7.take)(1)).subscribe((children) => {
  ...
  _expandAllItemsAtCurrentItemLevel() {
    ...
    let itemsToExpand;
    if (!parent) {
      itemsToExpand = (0, import_rxjs13.of)(this._items.filter((item) => item.getParent() === null));
    } else {
      itemsToExpand = coerceObservable(parent.getChildren());
    }
    itemsToExpand.pipe((0, import_operators7.take)(1)).subscribe((items) => {
...
var MatTableDataSource = class extends DataSource {
  ...
  _renderData = new import_rxjs15.BehaviorSubject([]);
  /** Stream that emits when a new filter string is set on the data source. */
  _filter = new import_rxjs15.BehaviorSubject("");
  /** Used to react to internal changes of the paginator that are made by the data source itself. */
  _internalPageChanges = new import_rxjs15.Subject();
  /**
   ...
   */
  filterPredicate = (data, filter4) => {
    const transformedFilter = filter4.trim().toLowerCase();
    return Object.values(data).some((value) => `${value}`.toLowerCase().includes(transformedFilter));
  };
  constructor(initialData = []) {
    super();
    this._data = new import_rxjs15.BehaviorSubject(initialData);
    this._updateChangeSubscription();
  }
  /**
  ...
  _updateChangeSubscription() {
    const sortChange = this._sort ? (0, import_rxjs15.merge)(this._sort.sortChange, this._sort.initialized) : (0, import_rxjs15.of)(null);
    const pageChange = this._paginator ? (0, import_rxjs15.merge)(this._paginator.page, this._internalPageChanges, this._paginator.initialized) : (0, import_rxjs15.of)(null);
    const dataStream = this._data;
    const filteredData = (0, import_rxjs15.combineLatest)([dataStream, this._filter]).pipe((0, import_operators9.map)(([data]) => this._filterData(data)));
    const orderedData = (0, import_rxjs15.combineLatest)([filteredData, sortChange]).pipe((0, import_operators9.map)(([data]) => this._orderData(data)));
    const paginatedData = (0, import_rxjs15.combineLatest)([orderedData, pageChange]).pipe((0, import_operators9.map)(([data]) => this._pageData(data)));
